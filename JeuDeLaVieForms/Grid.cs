...
namespace Jeu_de_la_vie
{
    class Grid
    {
        ...
        public Cell[,] lCells; // Tableau à deux dimensions contenant des objets de type Cell
        ...
        public Grid(int nbCells, List<Coords> AliveCellsCoords) // Constructeur de la class Grid
        {
            ...
            lCells = new Cell[n, n]; // Création d’un tableau à deux dimensions de taillen,n
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    ...
                    {
                        lCells[i, j] = new Cell(true);
                    }
                    else
                    {
                        lCells[i, j] = new Cell(false);
                    }
                }
            }
        }
        ...
        {
            int nbCells = 0;
            if (i - 1 >= 0 && lCells[i - 1, j].isAlive == true)
            {
                nbCells += 1;
            }
            if (i + 1 < n && lCells[i + 1, j].isAlive == true)
            {
                nbCells += 1;  
            }
            if (j - 1 >= 0 && lCells[i, j - 1].isAlive == true)
            {
                nbCells += 1;
            }
            if (j + 1 < n && lCells[i, j + 1].isAlive == true)
            {
                nbCells += 1;
            }
            if (j + 1 < n && i + 1 < n && lCells[i + 1, j + 1].isAlive == true)
            {
                nbCells += 1;
            }
            if (i - 1 >= 0 && j - 1 >= 0 && lCells[i - 1, j - 1].isAlive == true)
            {
                nbCells += 1;
            }
            if (i + 1 < n && j - 1 >= 0 && lCells[i + 1, j - 1].isAlive == true)
            {
                nbCells += 1;
            }
            if (i - 1 >= 0 && j + 1 < n && lCells[i - 1, j + 1].isAlive == true)
            {
                nbCells += 1;
            }
            return nbCells;
        ...
        public void DisplayGrid() // Méthode qui permet d’afficher une représentation de grille en console avec un X à chaque emplacement où une cellule est vivante
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    if (lCells[i, j].isAlive == true) { stateCell = "X"; } else { stateCell = " "; }
        ...
        public void UpdateGrid() // Méthode qui parcourt chaque cellule et qui met à jour leur attribut _nextStep, via son accesseur en écriture, en fonction des règles de la simulation.L’attribut est mis à true si la cellule reste en vie ou apparaît et à false si la cellule à cet emplacement disparaît ou reste absente. Une fois toute la grille parcourue, une deuxième passe est effectué pour associer la valeur de nexStep à l’attribut isAlive de chaque cellule.
        {
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    if(lCells[i, j].isAlive == true && getNbAliveNeighboor(i, j)==2 || getNbAliveNeighboor(i,j)==3)
                    {
                        lCells[i, j].ComeAlive();
                    }
                    else if (lCells[i, j].isAlive == false && getNbAliveNeighboor(i, j) == 3)
                    {
                        lCells[i, j].ComeAlive();
                    }
                    else
